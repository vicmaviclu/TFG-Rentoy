...
void main() {
  group('Tests de ControladorLogin (unidad)', () {
    ...
    test('inicio con email inválido devuelve Cadenas.emailInvalido', () async {
      final ctrl = ControladorLogin(
        signInWithEmail: (e, p) async => throw FirebaseAuthException(code: 'invalid-email'),
      );
      ...
      expect(res, Cadenas.emailInvalido);
    });

    // Simula error de contraseña incorrecta y verifica el mapeo a mensaje amigable
    test('inicio con contraseña incorrecta devuelve Cadenas.contrasenaIncorrecta', () async {
      final ctrl = ControladorLogin(
        signInWithEmail: (e, p) async => throw FirebaseAuthException(code: 'wrong-password'),
      );
      ...
      final res = await ctrl.iniciarSesionConEmail();
      expect(res, Cadenas.contrasenaIncorrecta);
    });

    // Simula intento de registro con contraseña débil (<6) y comprueba el mensaje
    test('registro con contraseña corta devuelve Cadenas.contrasenaCorta', () async {
      final ctrl = ControladorLogin(
        createUserWithEmail: (e, p) async => throw FirebaseAuthException(code: 'weak-password'),
      );
      ...
      expect(res, Cadenas.contrasenaCorta);
    });
  ...
  group('Tests de widget para FormularioRegistro', () {
    ...
    testWidgets('muestra SnackBar cuando las contraseñas no coinciden', (WidgetTester tester) async {
      final ctrl = ControladorLogin(
        createUserWithEmail: (e, p) async => _FakeUserCredential(),
      );
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: FormularioRegistro(controller: ctrl),
        ),
      ));
      ...
      // Tap register button
      await tester.tap(find.widgetWithText(ElevatedButton, Cadenas.registrarse));
      await tester.pump();

      // Should show a SnackBar with mismatch message
      expect(find.text(Cadenas.contrasenasNoCoinciden), findsOneWidget);
    });

    // Comprueba que tras un registro exitoso la pantalla se cierra (Navigator.pop)
    testWidgets('registro exitoso provoca Navigator.pop', (WidgetTester tester) async {
      final ctrl = ControladorLogin(
        createUserWithEmail: (e, p) async => _FakeUserCredential(),
      );
      ...
      expect(observer.didPopCalled, isTrue, reason: 'Expected the registration flow to pop the route after success');
    });
  });
}
